...
df = pd.read_json('https://api.upbit.com/v1/candles/minutes/5?market=KRW-BTC&count=200')
df_norm = df.drop(
    ['market', 'candle_date_time_utc', 'candle_date_time_kst', 'opening_price', 'high_price', 'low_price', 'timestamp',
     'candle_acc_trade_price', 'candle_acc_trade_volume', 'unit'], 1, inplace=True)
...
plt.plot(test_set[:, 0], color='red', label='Test Real')
